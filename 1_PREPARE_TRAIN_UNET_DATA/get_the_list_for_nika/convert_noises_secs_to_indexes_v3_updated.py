...
def main() -> int:
    ...
    if "filename" not in df.columns:
        raise ValueError(f"records_summary.csv must contain 'filename'. Found: {list(df.columns)}")

    rid_col = pick_recording_id_column(df)
    bname_col = pick_basename_column(df)

    df = df.copy()
    df["filename_norm"] = df["filename"].apply(_norm_name)

    # If duplicates exist, keep the last row for that filename_norm
    df_keyed = df.drop_duplicates(subset=["filename_norm"], keep="last")

    recid_map = df_keyed.set_index("filename_norm")[rid_col].to_dict()
    bname_map = df_keyed.set_index("filename_norm")[bname_col].to_dict()
    ...
    for filename, intervals in blocks.items():
        ...
        converted_blocks.append(f"filename = '{filename}'")
...
python convert_noises_secs_to_indexes_v3_updated.py \
  --noises_txt List_of_noises_secs_in_zive_records.txt \
  --summary_csv visi_zive_irasai_atrankai._modif_v1 - Darb.csv \
...
python convert_noises_secs_to_indexes_v3_updated.py \
  --noises_txt "List_of_noises_secs_in_zive_records.txt" \
  --summary_csv "visi_zive_irasai_atrankai._modif_v1 - Darb.csv" \
  --out_txt "noises_with_ids_full.txt" \
  --fs 200  
