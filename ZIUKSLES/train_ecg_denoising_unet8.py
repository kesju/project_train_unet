...
def read_selected_filenames(list_path=None):
    """Read optional list of ECG filenames to use. Empty or missing list means: use all files."""
    if list_path is None:
        logger.info("No ECG filename list provided; all .npy files from ecg_records_dir will be used")
        return None

    list_path = Path(list_path)
    if not list_path.exists():
        logger.error(f"ECG filename list does not exist: {list_path}")
        raise ValueError(f"ECG filename list does not exist: {list_path}")
    ...
    with list_path.open('r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            fname = line.strip()
            if fname:
                selected_filenames.append(os.path.basename(fname))

    if not selected_filenames:
        logger.info(f"ECG filename list is empty: {list_path}. All .npy files from ecg_records_dir will be used")
        return None

    selected_set = set(selected_filenames)
    logger.info(f"Loaded {len(selected_set)} unique filenames from ECG list: {list_path}")
    logger.debug("Selected filenames from list\n" + "\n".join(f"  - {fname}" for fname in sorted(selected_set)))
    return selected_set


def load_ecg_npy_files(directory, selected_filenames=None):
    """Load .npy ECG files and corresponding noise indices from JSON files, optionally filtering by filename list."""
    ...
    if selected_filenames is not None:
        logger.info(f"Filtering ECG files by provided filename list ({len(selected_filenames)} filenames)")
        file_paths = [path for path in file_paths if os.path.basename(path) in selected_filenames]
        if not file_paths:
            logger.error("None of the filenames from the provided list were found in ecg_records_dir")
            raise ValueError("None of the filenames from the provided list were found in ecg_records_dir")

        found_filenames = {os.path.basename(path) for path in file_paths}
        missing_filenames = sorted(selected_filenames - found_filenames)
        if missing_filenames:
            logger.warning(
                "The following filenames were listed but not found in ecg_records_dir\n" +
                "\n".join(f"  - {fname}" for fname in missing_filenames)
            )
...
def train_and_test_model(ecg_records_dir, model_output, vis_dir='visualizations', train_split=0.8, ecg_list_file=None):
    ...
    selected_filenames = read_selected_filenames(ecg_list_file)
    ecgs, noise_indices_list, filenames, patient_ids = load_ecg_npy_files(ecg_records_dir, selected_filenames)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ecg_list_file', type=str, default=None, help='Optional path to text file with ECG filenames to use; if omitted or empty, all .npy files from ecg_records_dir are used')
