...
- Sampling frequency (fs) is optional:
  - If you pass --fs, durations are computed.
  - If you omit --fs, the script will try to infer fs from the assumption that some records are ~10 min.
    (In your ZIP, inference typically yields 256 Hz because one record length is 153600 samples = 600 s * 256 Hz.)
"""
...
def infer_fs_from_records(sample_counts: List[int]) -> Optional[int]:
    """
    Best-effort inference of fs using the assumption that *some* records are ~10 min (600 s).

    Strategy:
    - For each record sample_count, compute fs_candidate = sample_count / 600.
    - Keep candidates that are within 1e-6 of an integer.
    - Return the most frequent integer candidate.

    If no candidates found, returns None.
    """
    candidates: List[int] = []
    for n in sample_counts:
        fs = n / 600.0
        r = round(fs)
        if abs(fs - r) < 1e-6 and r > 0:
            candidates.append(int(r))
    if not candidates:
        return None
    # mode, with stable fallback
    try:
        return statistics.mode(candidates)
    except statistics.StatisticsError:
        # multiple modes -> pick the largest (often safer for ECG)
        return max(set(candidates), key=lambda x: (candidates.count(x), x))
...
def analyze(input_path: Path, out_dir: Path, fs: Optional[int]) -> None:
    ...
    def run_analysis(
        ...
        # infer fs if not provided
        if fs is None:
            sample_counts: List[int] = []
            for seq in sequences:
                for _, _, bin_member in list_pairs_fn(seq):
                    bsz = file_size_fn(bin_member)
                    if _is_int16_sized(bsz):
                        sample_counts.append(_samples_from_int16_bytes(bsz))
            fs_inferred = infer_fs_from_records(sample_counts)
            fs = fs_inferred
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--fs", type=int, default=None, help="Sampling frequency in Hz (optional). If omitted, tries to infer.")
...
# python3 analyze_ecg_zip_structure.py user-65955b5f50e02b125d4998ad --out ecg_zip_report --fs 200